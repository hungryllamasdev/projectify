...
export default function Calendar({ data: initialData }: CalendarProps) {
  ...
  return (
    <div className={`flex ${isMobile ? "flex-col" : "flex-row"} gap-8`}>
      <Card className={`mb-8 ${isMobile ? "w-full" : "flex-1"}`}>
            ...
            {monthDays.map((day, dayIdx) => {
              ...
              return (
                <Button
                  ...
                  variant="outline"
                  className={`
                                        h-16 sm:h-20 p-1 sm:p-2 flex flex-col items-start justify-start
                ...
                >
                  ...
                  <div className="flex flex-wrap gap-1 mt-1">
                    {dayTasks.slice(0, 3).map((task) => (
                      <div
                        key={task.id}
                        className={`w-1 h-1 sm:w-2 sm:h-2 rounded-full ${statusColors[task.status]}`}
                        title={task.title}
                      ></div>
                    ))}
